...
> => {
  ...
  const StakingRewardsProvider: FC<{ address?: string; stakingTokenAddress?: string; mAssetName?: MassetName }> = ({
    ...
    const stakingRewardsContract = useMemo<StakingRewardsContract | undefined>(() => {
      ...
      const data =
        (query as unknown as NonNullable<StakingRewardsContractQueryResult>).data?.stakingRewardsContract ??
        (query as unknown as NonNullable<StakingRewardsForStakingTokenQueryResult>).data?.stakingRewardsContracts?.[0]
