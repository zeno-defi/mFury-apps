        ...
        {showGraph ? (
              ...
              <CustomTable headerTitles={headerTitles}>
                {rewardStreams?.lockedStreams?.map(stream => (
                  <TableRow key={stream.start} buttonTitle="View">
                    ...
                    <TableCell>
                      <p>{(stream.amount?.[2] ?? stream.amount?.[3])?.toFixed(2)} FURY</p>
                    </TableCell>
                  </TableRow>
                ))}
              </CustomTable>
          ...
          <EmptyState>
            ...
            {selectedSaveVersion === 1 ? (
              <p>Migrate your balance and deposit to the Vault to earn FURY rewards.</p>
            ) : (
              <p>Deposit to the Vault to earn FURY rewards.</p>
            )}
          </EmptyState>
        )}
      </div>
