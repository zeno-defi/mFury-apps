...
const ClaimFormRewards: FC = () => {
  ...
  return <StyledMultiRewards rewardsEarned={{ rewards: [{ earned: BigDecimal.fromSimple(rewardsEarned?.rewards ?? 0), token: 'FURY' }] }} />
}
...
export const ClaimForm: FC = () => {
  ...
  return (
    <Container>
      <ClaimFormRewards />
      {stakedTokenSymbol === 'FURY' ? (
        <Compound>
          <div>
            <h3>Compound rewards?</h3>
            <ToggleInput onClick={toggleIsCompounding} checked={isCompounding} />
          </div>
          <p>This will claim and re-stake your earned FURY in 1 transaction</p>
        </Compound>
      ) : (
        <MerkleDropBAL />
      )}
